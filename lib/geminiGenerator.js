...
/**
 * Generates a hero image using Unsplash API
 * Finds high-quality stock photos matching the prompt
 */
export async function generateHeroImage(imagePrompt, title = '') {
  try {
    const timestamp = Date.now();
    const scriptPath = `/tmp/image_unsplash_${timestamp}.py`;
    const outputPath = `public/generated/image_${timestamp}.jpg`;
    ...
    // Create Python script to fetch from Unsplash
    const pythonScript = `
import requests
import os
...
try:
    ...
    # Unsplash API (free, no auth needed for basic usage)
...
`;
    ...
    if (stdout.includes('SUCCESS')) {
      const imageUrl = `/generated/image_${timestamp}.jpg`;
      ...
      console.error('Image fetch failed:', stderr || stdout);
      return {
        success: false,
