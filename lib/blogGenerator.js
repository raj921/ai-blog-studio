...
export async function generateBlogPost({
  ...
  try {
    ...
    // Build the Python script using string concatenation to avoid template literal issues
    const pythonScript = `
import asyncio
import json
import os
...
async def generate_blog():
    try:
        ...
        chat = LlmChat(
            api_key=api_key,
            session_id="blog-` + timestamp + `",
            system_message="You are a professional blog writer specializing in creating well-structured, engaging content. You write in a ` + tone + ` tone and create content that is SEO-friendly while remaining informative and valuable to readers."
        ...
        try:
            ...
            fallback_content = {
                "title": "` + title + `",
                "introduction": "Generated content for ` + topic + `. " + str(response)[:200] + "...",
                "sections": [
                    {
                ...
                "imagePrompt": "Professional illustration related to ` + topic + `"
            }
        ...
        error_response = {
            "error": str(e),
            "title": "` + title + `",
            "introduction": "We apologize, but we are currently unable to generate the full content for ` + topic + `. Please try again later.",
            "sections": [
                {
            ...
            "imagePrompt": "Abstract professional image related to ` + topic + `"
        }
...
`;
    ...
    // Use configurable Python path or fallback to system python3
...
export async function generateHeroImage(imagePrompt, title = '') {
  try {
    ...
    const outputPath = path.join(process.cwd(), 'public', 'generated', `image_${timestamp}.png`);
...
`;
    ...
    if (stdout.includes('SUCCESS')) {
      ...
      // Validate that the image file was actually created
      const fullPath = path.join(process.cwd(), 'public', imageUrl);
      if (fs.existsSync(fullPath)) {
        const stats = fs.statSync(fullPath);
        const fileSizeInMB = stats.size / (1024 * 1024);
        
        // Validate file size (should be reasonable for an image, max 10MB)
        if (fileSizeInMB > 10) {
          console.error(`Image file too large: ${fileSizeInMB.toFixed(2)}MB`);
          return { success: false, error: 'Generated image file is too large' };
        }
        
        console.log('Hero image generated successfully');
        return { success: true, imageUrl };
      } else {
        console.error('Image file was not created at expected location');
